...
def diagrama_bloco_5kg():
    ...
    # Peso para baixo
    ax.arrow(-0.36, -0.5, 0, -1.2, head_width=0.18, head_length=0.22, fc="#ff8800", ec="#ff8800", lw=3, length_includes_head=True)
    ax.text(-0.7, -1.85, r"$5kg\times9.8\ m/s^2$", fontsize=14, color="black", ha='center', va='center')

    # Tensão da corda para baixo (seta azul abaixo do bloco)
    ax.arrow(0.36, -0.5, 0, -1.6, head_width=0.15, head_length=0.20, fc="blue", ec="blue", lw=3, length_includes_head=True)
    ax.text(0.45, -2.0, "Tensão da corda", fontsize=14, color="black", ha='left', va='center')

    # Força aplicada F para cima (seta/texto verde acima e deslocado)
    ax.arrow(0.3, 0.5, 0, 1.3, head_width=0.18, head_length=0.22, fc="green", ec="green", lw=3, length_includes_head=True)
    ax.text(0.7, 2.0, r"Força aplicada $F=200N$", fontsize=17, color="green", ha='center', va='center')
